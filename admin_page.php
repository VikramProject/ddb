      ...
      <?php
         ...
         while ($obj = $result->fetch_object())
             ...
             <div class=\"col-sm-12 head-containerbs3\">
               ...
               <div class=\"col-sm-10\">          
         		...
         		if ($count == 1) echo "<div id='count' style='margin-top: 10px;text-align: left;color: red;font-style: italic'>$count form left in the book</div>";
         		  else echo "<div id='count' style='margin-top: 10px;text-align: left;color: red'>$count forms left in the book</div>";
         		echo "</div>
                     </div></div>
             </div>
         ...
         <div class=\"row\" >
             <div class=\"col-sm-6 column-details left-column-details\"> <!-- left column -->
                 ...
                 <div class=\"row\">
                     <div class=\"col-sm-12\">
                         <span class=\"value-details\">" . date("d-m-Y", strtotime($obj->DOB)) . "</span>
                     </div>
                 </div>
                 <!-- address -->
                 <div class=\"row\">
...
<script src="assets/js/ie10-viewport-bug-workaround.js"></script>s
<script type="text/javascript" language="javascript">
   $(document).ready(function () {
       $(".approve").click(function () {
           ...
           if (ser > end) {
               alert(
                   "The serial number provided is not registered in the current concession book. Please insert the correct serial number"
               );
               ser1.val(curr);
               return
           };
           if (ser < curr) {
               alert(
                   "The serial number provided has already been used. Please provide an appropriate serial number"
               );
               ser1.val(curr);
               return;
           }
           ...
           var age = $(this).parents('.modal').find('#age').text();
           if (ser.length <= 0) {
               ...
               $.ajax({
                   ...
                   success: function (data) {
                       ...
                       if (left == 1)
                           $('.modal').find('#count').html(left +
                               " form left in the book");
                         else
                           $('.modal').find('#count').html(left +
                               " forms left in the book");
                   }
               });
           }
       });
